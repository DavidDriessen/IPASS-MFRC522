...
bool ArduinoConsole::Read(char *data, int &count) {
    char n = 0;
    count = 0;
    while (n != 13) {
        if (n == 8 && count > 0) {
...
void ArduinoConsole::run() {
    ...
    if (!Read(serialInData, serialInCount)) {
        return;
    }
    ...
    if (serialInCount != 0) {
        ...
        hwlib::cout << "No command found:";
        for (int l = 0; l < serialInCount; ++l) {
            hwlib::cout << serialInData[l];
        }
        hwlib::cout << "\n";
    }
}
